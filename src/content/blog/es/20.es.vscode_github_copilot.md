...
```
De esta forma queda explicito qué instrucciones seguir. Y, sí, también se puede hacer simplemente agregando el fichero o ficheros de instrucciones al contexto del chat, pero entonces solo aplica en ese momento y no es fácil reutilizarlo; que es lo siguiente que veremos.
...
- Al terminar, confirma el resultado ejecutando el comando [git-commit.prompt.md](.github/prompts/git-commit.prompt.md)
```

Como ves, los **archivos de prompt** pueden referenciar otros archivos del proyecto usando sintaxis Markdown estándar o la sintaxis específica `#file:ruta/archivo.ts`. Esto permite proporcionar contexto adicional como especificaciones de API o documentación del producto.
